<?php namespace App\Modules\CustomerCare\Controllers;
...
class CustomerCareController extends Controller {
    ...
    public function insertCcPreSalesRemark()
    {
        ...
        public function ccfilter()
        {
            ...
            {
                $source_id = explode("_", $filter["model_id"]);
                $filter["model_id"] = $source_id[0];
            }   
            else 
            {
                $filter["model_id"]  = "";
            }
            if (!empty($filter["test_drive_given"])) {
                $testdrivestatus = explode("_", $filter["test_drive_given"]);
                $filter["test_drive_given"] = $testdrivestatus[0];
            }
            else
            {
                $filter["test_drive_given"] = '';
            }    
            ...
            $filter["test_drive_given"] = (!empty($filter['test_drive_given']) || isset($filter['test_drive_given'])) ? $filter['test_drive_given'] : "";
            ...
            $enquiries = DB::select('CALL ' . $request["getProcName"] . '("' . $loggedInUserId.'",' . $request['pageNumber'] . ',' 
                                            ...
                                            . $filter["source_id"] . '","' . $filter["subsource_id"] . '","' . $filter['model_id'] . '","' 
                                            . $filter["fromDate"] . '","' . $filter["toDate"] . '","' . $filter["test_drive_given"] . '","' 
