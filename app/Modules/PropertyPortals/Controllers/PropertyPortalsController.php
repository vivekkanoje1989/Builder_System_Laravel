...
class PropertyPortalsController extends Controller {
    ...
    public function properyPortalAccount() {
        ...
        $result = ['success' => true, 'records' => $portalAccounts, 'portalName' => $portalName, 'exportData' => $export];
        echo json_encode($result);
    }

    public function accountsExportToxls($id) {
        $array = json_decode(Auth::guard('admin')->user()->employee_submenus, true);
        if (in_array('01401', $array)) {
            $result = LstPropertyPortalsConfig::where('property_portal_id', $id)->get();
            $getCount = LstPropertyPortalsConfig::where('property_portal_id', $id)->get()->count();
            $accounts = array();
            $j = 1;
            $manageAccount = json_decode(json_encode($result), true);
            for ($i = 0; $i < count($manageAccount); $i++) {
                $account['Sr No'] = $j++;
                $account['Friendly Account Name'] = $manageAccount[$i]['portal_name'];
                $account['Assign Enquiries to'] = $manageAccount[$i]['employee_id'];
                if ($manageAccount[$i]['status'] == 1) {
                    $account['Assign Enquiries to'] = 'Yes';
                } else {
                    $account['Assign Enquiries to'] = 'No';
                }
                $accounts[] = $account;
            }
            ...
            if ($getCount < 1) {
                ...
                Excel::create('Export Accounts Data', function($excel) use($accounts) {
