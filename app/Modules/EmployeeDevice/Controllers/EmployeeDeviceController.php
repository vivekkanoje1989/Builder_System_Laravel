...
class EmployeeDeviceController extends Controller {
    ...
    public function employeeDeviceExportToxls() {
        $array = json_decode(Auth::guard('admin')->user()->employee_submenus, true);
        if (in_array('01401', $array)) {
            $getCount = EmployeesDevice::select('employee_id', 'device_mac', 'device_name', 'device_status', 'device_type', 'id')->get()->count();
            ...
            for ($i = 0; $i < count($data); $i++) {
                $employeeDevice['Sr No'] = $j++;
                $employeeDevice['Device Name'] = $data[$i]['device_name'];
                $employeeDevice['Mac Address'] = $data[$i]['device_mac'];
                $employeeDevice['Employee Name'] = $data[$i]['employee_id'];
                if ($data[$i]['device_type'] == '1') {
                    $employeeDevice['Device Type'] = 'desktop';
                } else if ($data[$i]['device_type'] == '2') {
                    $employeeDevice['Device Type'] = 'laptop';
                } else {
            ...
            if ($getCount < 1) {
                ...
                Excel::create('Export Employee Device Data', function($excel) use($employeeData) {
