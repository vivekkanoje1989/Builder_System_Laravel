...
class PaymentHeadingsController extends Controller {
    ...
    public function paymentHeadingExportToxls() {
        $array = json_decode(Auth::guard('admin')->user()->employee_submenus, true);
        if (in_array('01401', $array)) {
              $getPayment = LstDlPaymentHeadings::select('payment_heading')->get();
            $getCount = LstDlPaymentHeadings::select('payment_heading')->get()->count();
            ...
            if ($getCount < 1) {
                ...
                Excel::create('Export Block Stage Details', function($excel) use($managePayment) {
