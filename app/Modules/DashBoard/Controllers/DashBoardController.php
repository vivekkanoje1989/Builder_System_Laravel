...
//            $request['filterData']['in_date'] = !empty($request['filterData'][0]['in_date']) ? date('Y-m-d', strtotime($request['filterData'][0]['in_date'] )): '';
                ...
                $report = "SELECT req.in_date, req.created_at, req.request_type, req.from_date, req.req_desc, req.to_date, req.status,emp.first_name,emp.last_name "
                        . "FROM `request` as req LEFT JOIN employees as emp ON emp.id = req.uid "
                        . "WHERE req.created_by = ".$loggedInUserId.$query. " limit " . $startFrom . "," . $request['itemPerPage'];
                $employees = DB::select($report);
            } else {
                $report = "select SQL_CALC_FOUND_ROWS request.id, request.in_date, request.created_at, request.request_type, request.from_date, request.req_desc, request.to_date, employees.first_name, employees.last_name, request.status from request left join employees on find_in_set(employees.id, request.uid) where request.created_by = ".$loggedInUserId." limit " . $startFrom . "," . $request['itemPerPage'];
                $employees = DB::select($report);
            }
            $rows = DB::select("select FOUND_ROWS() as totalCount");
            $cnt = $rows[0]->totalCount;
        } else {
            $report = "select request.id, request.in_date, request.created_at, request.request_type, request.from_date, request.req_desc, request.to_date, employees.first_name, employees.last_name, request.status from request left join employees on find_in_set(employees.id, request.uid) where request.created_by = ".$loggedInUserId;
            $employees = DB::select($report);
            $cnt = '';
        }
        $i = 0;
...
//        }
        if (!empty($employees)) {
            $result = ['status' => true, 'records' => $employees, 'exportData' => 1, 'totalCount' => $cnt];
        } else {
            $result = ['status' => false, 'message' => "No record"];
        }
        return json_encode($result);
