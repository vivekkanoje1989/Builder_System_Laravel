...
class DiscountHeadingsController extends Controller {
    ...
    public function manageDiscountHeadings() {
        ...
        $getDiscounts = LstDlDiscounts::take($request['itemPerPage'])->offset($startFrom)->get();
        $getDiscountname = LstDlDiscounts::all();

        if (!empty($getDiscounts)) {
            $result = ['success' => true, 'records' => $getDiscounts, 'totalCount' => count($getDiscountname)];
