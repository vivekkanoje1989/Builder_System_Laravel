...
class MasterSalesController extends Controller {
    ...
    public function create() {
        return view("MasterSales::index")->with(["editEnquiryId" => '']);
    }
    ...
    public function store() {
        try {
            ...
            $corporate_customer = (isset($input['customerData']['corporate_customer']) && $input['customerData']['corporate_customer'] == 'true') ? '1' : '0';
            ...
            $input['customerData']['corporate_customer'] = $corporate_customer;
    ...
    public function editCustomer($cid) {
        return view("MasterSales::index")->with(["editCustomerId" => $cid, "editEnquiryId" => '']);
    }
        ...
        try {
            ...
            if (count($getCustomerContacts) > 0) {
                ...
                if (count($getCustomerEnquiryDetails) == 0 || isset($request['data']['showCustomer'])) {
                    $result = ['success' => true, 'customerPersonalDetails' => $getCustomerPersonalDetails, 'customerContactDetails' => $getCustomerContacts, 'flag' => 0, 'outBoundCall' => $outBoundCall, 'displayMobile' => $displayMobile];
                } else {
                    $result = ['success' => true, 'customerPersonalDetails' => $getCustomerPersonalDetails, 'customerContactDetails' => $getCustomerContacts, 'CustomerEnquiryDetails' => $getCustomerEnquiryDetails, 'flag' => 1, 'outBoundCall' => $outBoundCall, 'displayMobile' => $displayMobile];
                }
            } else {
...
//            $request['enquiryData']['sales_employee_id'] = $loggedInUserId;
        $request['enquiryData']['sales_employee_id'] = $request['enquiryData']['followup_by_employee_id'];
        ...
        if ($insertEnquiry) {
            ...
            $request['followupDetails']['followup_by_employee_id'] = $loggedInUserId;
            ...
            // if new reassignTo then insert follow up and send reassign mail
            if ($loggedInUserId != $request['enquiryData']['followup_by_employee_id']) {
                $request['followupDetails']['followup_by_employee_id'] = $request['enquiryData']['followup_by_employee_id'];
                $emp = Employee::select('first_name', 'last_name')->where('id', $request['enquiryData']['followup_by_employee_id'])->get();
                $request['followupDetails']['remarks'] = Auth::guard('admin')->user()->first_name . " " . Auth::guard('admin')->user()->last_name . " reassig enquiry to " . $emp[0]->first_name . " " . $emp[0]->last_name;
                $Enq = EnquiryFollowup ::create($request['followupDetails']);
                // reassign template                    
                $templatedata['employee_id'] = $request['enquiryData']['followup_by_employee_id'];
...
" . $getCallBackNo[0]['virtual_display_number'] . "";
    ...
    public function getTeamSharedEmployee() {
        ...
        foreach ($allEmpresult as $emp) {
            if (!empty($emp['presale_shared_employee'])) {
                if (!empty($sharedEmployees)) {
                    $sharedEmployees = $sharedEmployees . "," . $emp['presale_shared_employee'];
                } else {
                    $sharedEmployees = $emp['presale_shared_employee'];
                }
            }
        }
        if (!empty($sharedEmployees)) {
            $arr = array_unique(explode(',', $sharedEmployees));
            $teamshared = implode(',', $arr);
        } else {
            $teamshared = '';
        }
        $alluser = implode(',', $alluser);
        $result = ['success' => true, 'teamshared' => $teamshared, 'alluser' => $alluser];
        return json_encode($result);
    }
    ...
    public function getTotalEnquiries() { // get all enquiries
        try {
            ...
            if ($request['teamType'] == 0) { // total
                ...
                if (empty($request['empId'])) {
                    $employees = json_decode($this->getTeamSharedEmployee());
                    if (!empty($employees->teamshared) && $request['shared'] == '1') {
                        $loggedInUserId = $employees->teamshared;
            ...
            $excel->sheet($reportName, function($sheet) use ($data, $reportName) {
                ...
                foreach ($data as $user) {
                    ...
                    $getFilterData = [$user["sales_enquiry_date"], $user["customer_fname"] . " " . $user["customer_lname"],
                        ...
                        $user["sales_status"], $user["sales_source_name"], $user["enquiry_sub_source"],
        ...
        try {
            ...
            if (!empty($doc['specification_images']) && count($doc['specification_images']) > 0) {
                ...
                foreach ($doc['specification_images'] as $spec) {
                    $specific = $specific . "<a href='https://storage.googleapis.com/bkt_bms_laravel/project/specification_images/" . $spec['specification_images'] . "' ><img src='https://storage.googleapis.com/bkt_bms_laravel/project/specification_images/" . $spec['specification_images'] . "' style='margin-right:8px;height:120px;width:120px;'></a>";
                }
            }
